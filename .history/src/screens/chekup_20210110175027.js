...
export const ChekupScreen = (props) => {
  ...
  const handleFileGet = () => {
    ...
    // fileData.map((item) => {
      ...
      // setFileDataItem({name: item.name,type:item.name.split('.').pop(),date:item.createdAt});
      // setFileDataItem({type:item.name.split('.').pop()});
      // setFileDataItem({date:item.createdAt});
    // })
    // setFileDataItem({name:fileItem.name,type:fileItem.type,date:fileItem.date});
    // fileDataItem.map((item)=>{
